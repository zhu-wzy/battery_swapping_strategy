...
def run_ga_matching_final(nodes_file, speeds_file, output_csv='ga_final_result.csv'):
    print(f"🔹 正在读取数据...")
    try:
    ...
    station_num_chargers = np.zeros(num_stations)
    ...
    for i, (idx, row) in enumerate(stations_sorted.iterrows()):
        ...
        wq_hours = mmn_analytical(lam, mu, params['N'])
        station_wait_times[idx] = wq_hours * 60.0
        station_num_chargers[idx] = params['N']

    # 计算边际排队成本 (用于最后计算 Std)
    # 增加1辆车增加的排队时间 = 60 / (N*mu) 分钟
    station_marginal_costs = 60.0 / (np.maximum(station_num_chargers, 1) * mu)
    ...
    for gen in range(GENERATIONS):
        ...
        selected = [x[0] for x in scores[:POP_SIZE//2]]
    ...
    for s_idx in global_best_ind:
        station_counts[s_idx] += 1

    # 修正后的排队时间 = 基础排队 + (排队人数 * 边际时间)
    adjusted_wait_times = station_wait_times + (station_counts * station_marginal_costs)

    # 负载均衡度 (Load Std)
    load_std = np.std(adjusted_wait_times)

    print(f"✅ 优化完成!")
    print(f"   ⏱️ 时间总成本: {global_best_cost:.2f} min")
    print(f"   ⚖️ 负载均衡度 (Std): {load_std:.4f}")
    ...
    plt.title(f'遗传算法总成本下降曲线\nTotal Cost: {global_best_cost:.1f} | Load Std: {load_std:.2f}')
    plt.xlabel('迭代次数')
    plt.ylabel('总成本 (min)')
    ...
    plt.title(f'最优匹配方案\n总成本: {global_best_cost:.1f} min | 负载均衡度(Std): {load_std:.2f}')
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ...
    if os.path.exists(node_file) and os.path.exists(speed_file):
        run_ga_matching_final(node_file, speed_file)
    else:
        print("❌ 错误：找不到输入文件。")
